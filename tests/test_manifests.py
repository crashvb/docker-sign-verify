...
@pytest.fixture()
def formattedsha256():
    return FormattedSHA256("ABCDEF0123456789ABCDEF0123456789ABCDEF0123456789ABCDEF0123456789")
...
def test_init(archive_manifest: ArchiveManifest, devicemapper_repository_manifest: DeviceMapperRepositoryManifest,
...
def test_str(archive_manifest: ArchiveManifest, devicemapper_repository_manifest: DeviceMapperRepositoryManifest,
             registry_v2_manifest: RegistryV2Manifest, manifest_archive: bytes, manifest_registry: bytes,
    ...
    assert json.loads(str(devicemapper_repository_manifest)) == json.loads(manifest_repository)
    assert json.loads(str(registry_v2_manifest)) == json.loads(manifest_registry)


def test_archive__layer_to_hash(archive_manifest: ArchiveManifest, sha256_archive_layer: str,
...
def test_archive__hash_to_layer(archive_manifest: ArchiveManifest, sha256_archive_layer: str,
                                formattedsha256: FormattedSHA256):
    """Test hash to layer conversion."""
    assert archive_manifest.digest_to_layer(formattedsha256) == sha256_archive_layer


@pytest.mark.parametrize("img_name,config_value", [
...
def test_archive__get_config(archive_manifest: ArchiveManifest, img_name: str, config_value: str):
...
def test_archive_append_config(archive_manifest: ArchiveManifest, sha256_archive_layer: str,
                               formattedsha256: FormattedSHA256, repotag):
    """Test appending configurations."""
    layers = [formattedsha256]
    archive_manifest.append_config(formattedsha256, layers, repotag)
    image_name = ImageName(None, formattedsha256.sha256, None)
...
def test_archive_get_config_digest(archive_manifest: ArchiveManifest):
    ...
    formattedsha256 = FormattedSHA256("a51f3f9281a1a3d89dce25fec8acffbe9f59ddb67d98e04245c4c886e32d3782")
...
def test_registry_override_config(registry_v2_manifest: RegistryV2Manifest, formattedsha256: FormattedSHA256):
...
def test_registry_get_config_digest(registry_v2_manifest: RegistryV2Manifest):
    formattedsha256 = FormattedSHA256("8f1196ff19e7b5c5861de192ae77e8d7a692fcbca2dd3174d324980f72ab49bf")
    assert registry_v2_manifest.get_config_digest() == formattedsha256
    assert registry_v2_manifest.get_config_digest("ignored") == formattedsha256


def test_registry_get_layers(registry_v2_manifest: RegistryV2Manifest):
    """Test manifest layer retrieval."""
    layers = ["sha256:6c8c72249e560701aa1da4cd40192274a8c0419ddb8e4a553aa02b5a1acdb863",
              "sha256:1403b179e2c9df4f57e9ea94e32882739c6b3d75ed756d4e67fcc424288c29cc"]
    assert registry_v2_manifest.get_layers() == layers
    assert registry_v2_manifest.get_layers("ignored") == layers


@pytest.mark.parametrize("img_name", ["endpoint:port/namespace/image:tag", "rancher/swarmkit:v1.13.0-beta.1"])
def test_repository_override_config(devicemapper_repository_manifest: DeviceMapperRepositoryManifest,
                                    formattedsha256: FormattedSHA256, img_name: str):
    """Test overriding manifest values."""
    image_name = ImageName.parse(img_name)
    devicemapper_repository_manifest.override_config(formattedsha256, image_name)
    assert devicemapper_repository_manifest.get_config_digest(image_name) == formattedsha256

@pytest.mark.parametrize("img_name,config_digest", [
    ("busybox:latest", "sha256:6ad733544a6317992a6fac4eb19fe1df577d4dec7529efec28a5bd0edad0fd30")
])
def test_repository_get_config_digest(devicemapper_repository_manifest: DeviceMapperRepositoryManifest, img_name: str, config_digest: str):
    image_name = ImageName.parse(img_name)
    assert devicemapper_repository_manifest.get_config_digest(image_name) == config_digest
